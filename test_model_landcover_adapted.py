...
def do_args(arg_list, name):
    ...
    parser.add_argument("--aug_model", action="store", dest="aug_model_fn", type=str, required=True, \
        help="Path to sklearn .model file to use"
    )
...
def main():
    ...
    aug_model_fn = args.aug_model_fn
    ...
    tmodel = keras.models.load_model(model_fn, custom_objects={
    ...
    model = keras.models.Model(inputs=tmodel.inputs, outputs=[tmodel.layers[feature_layer_idx].output])
    ...
    aug_model = joblib.load(aug_model_fn)
    ...
    for i in range(len(pair_list)):
        ...
        t_shape = output.shape
        output = output.reshape(-1, output_shape[2])
        output = aug_model.predict_proba(output)
        output = output.reshape(t_shape[0], t_shape[1], 4)

        output = np.concatenate([
            np.zeros((output.shape[0], output.shape[1], 1)),
            output
        ], axis=2)
